...
<%
ArrayList<Facture> commandes =(ArrayList<Facture>)request.getAttribute("ListFacture");
			if (commandes != null) {
				...
				for (int i = 0; i < commandes.size(); i++) 
				{
					...
					out.println("<td> " + commandes.get(i).getCommande().getDate_livraison()+ "</td>");
					out.println("<td> " + commandes.get(i).getCommande().getMtFinal()+ "</td>");
				    out.print("<td>  <a class='btn btn-warning btn-sm' target='_blank' href ='Facture?id="+ commandes.get(i).getCommande().getId()+"'><span class='glyphicon glyphicon-paste'></span> Facture</a> ");
							%>   <a target="_blank" href="ViewProduitCommande?id=<%out.print(commandes.get(i).getCommande().getId()); %>" class="btn btn-primary pull-right"  >
