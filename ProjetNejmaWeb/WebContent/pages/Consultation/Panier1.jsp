          ...
          <div class="box">
            <div class="box-header">
              <center><h3 class="box-title">Mon panier</h3></center>
                <ol class="breadcrumb">
                 <a type="button" href="ViderPanier1" class="btn btn-success btn-lg btn3d"><span class="glyphicon glyphicon-folder-close"></span> Vider panier</a>
     
        <ul class="nav navbar-nav navbar-right">
                
     <a type="button" href="ConsulterListeProduitClientRange" class="btn btn-success btn-lg btn3d"><span class="glyphicon glyphicon-hand-left"></span> Contenu Achat</a>
...
Panier prods =(Panier)session.getAttribute("panier");
			if (prods != null) {
				...
				for (int i = 0; i < prods.getProduits().size(); i++) 
				{
					...
					out.println("<td><img width='120px' heigth='120px' src='images/"+((Produit) prods.getProduits().get(i)).getImage()+"'/></td>");
					
					
					out.println("<td> <a class='btn btn-danger btn-sm'  href ='SupprimerPanier1?id="+((Produit) prods.getProduits().get(i)).getId()+"'  onclick='return confirm(\"Voulez vous vraiment supprimer ce produit ?\")'      ><span class='glyphicon glyphicon-trash'></span>  Supprimer produit</a> </td>");
...
<form  action="ValiderPanier1" method="POST" name="userForm">
 ...
 <div class="form-group" ng-class="{ 'has-error' : userForm.pts.$invalid && !userForm.pts.$pristine }">   
   ...
   <div class="form-group" ng-class="{ 'has-error' : userForm.typep.$invalid && !userForm.typep.$pristine }">   
    <label for="nm" >Choisissez votre type de paiement :</label>
    <select id="c100" name="typep" class="form-control"  ng-model="user.typep" required>
    <option></option>
    <option></option>
    <option>paieyment en ligne</option>
    <option>payment par especes</option>
   
    </select>
      <p ng-show="userForm.typep.$error.minlength" class="help-block">Choisir le type.</p>
